...
// Посадка подшипника 698 ZZ (под сальник 8×19×7)
bearing_od = BEARING698_OD;          // наружный диаметр
bearing_id = BEARING698_ID;           // внутренний диаметр
bearing_w  = BEARING698_W;            // ширина (высота)
bearing_fit = BEARING698_FIT;         // зазор под печать
bearing_shoulder = BEARING698_SHOULDER;   // упор подшипника

// Обойма подшипника: глубина 15мм, углубление в дне 16×1мм, канавка под стопорное кольцо
housing_depth = 15.7;
housing_recess_d = 16.7;
housing_recess_h = 1;
groove_d = bearing_od+bearing_fit+2*0.5;      // диаметр канавки (глубина 0.5мм при обойме 19.2мм)
groove_h = 1.2;       // высота канавки (толщина стопорного кольца)
groove_z = 13.5;      // высота канавки от дна обоймы
...
module flow_straightener_assembly(
    ...
    difference() {
        ...
        // Облегчение ступицы начинаем выше зоны подшипника (обойма 15мм + shoulder)
        wall = 2.0;

        inner_start = housing_depth + bearing_shoulder;
        ...
        // Обойма под 698 ZZ: глубина 15мм, углубление в дне 16×1мм, канавка под стопорное кольцо
        translate([0,0,-0.05])
            cylinder(h = housing_depth + 0.1, d = bearing_od + bearing_fit, center = false, $fn = 96);

        // Углубление по центру дна обоймы (16мм × 1мм)
        translate([0,0,housing_depth-0.05])
            cylinder(h = housing_recess_h + 0.05, d = housing_recess_d,center = false, $fn = 96);

        // Канавка под стопорное кольцо (20.2мм × 1.2мм на высоте 13.5мм)
        translate([0,0,housing_depth - groove_z - groove_h])
            cylinder(h = groove_h + 0.05, d = groove_d,center = false, $fn = 96);

        // Сквозное отверстие под ось 8 мм
        translate([0,0,-0.05])
            cylinder(h =inner_start -1, d = bearing_id + 0.2, center = false, $fn = 64);
...
// Толщина уха и логичное утолщение сектора кольца
// Дефолт задаем как undef, чтобы избежать проблем с вычислением дефолта
// через plate_thickness на некоторых версиях OpenSCAD.
ear_thickness = undef,

// Углубление под шляпку болта (шестигранник под ключ)
...
//    %translate([0, 0, -10])
    ...
    rotate([180,0,0])
    adapter_plate();
    adapter_plate(bolt_head_depth = 3);
}
